<div>
    @props(['thread' => $thread])
    <a href="{{ route('user.threads.detail', $thread) }}" class="p-4 block grid bg-white sm:rounded-lg border-1 shadow hover:bg-gray-100 duration-100">
    ...
    <div class="gap-x-2 flex pl-2 mb-2">
        @if(auth('users')->user()->id === $thread->user_id)
